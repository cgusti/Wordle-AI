...
public class EditDistanceUtils {
	...
	public static int[][] getEditDistTable(String s0, String s1) {
        ...
        // >> [AF] Better than having iterator named "i", how about "col" so it's
        // clear that you're completing the columns here?
		...
		for (int row = 1; row < s0.length() + 1; row++) {
			for (int col = 1; col < s1.length() + 1; col++) {
				...
				// case 2: case mismatch
					...
					// transposition
                    // >> [AF] Sloppy to use a hard-coded magic-number, what if the dist were > 1000? Better: use some
                    // data structure to collect the values, then pick the minimal from it. At the very least, should've
    ...
    // >> [AF] Javadocs must be placed *directly* above the methods they document, and if
    // non-void, should specify the @return value.
	/*
	...
	public static List<String> getTransformationList(String s0, String s1, int[][] table) {
		...
		while ( !(row == 0 && col == 0) ) {
            // >> [AF] Lots of extraneous spaces here -- be judicious with newlines, never need more than 2
            // in a row, let alone more than 1 *within* a single method.
			// case match
			...
			// case mismatch
                ...
                // >> [AF] Lots of code repetition below -- see the currentPosition and continue in each if-statement? (-0.5)
	...
	public static int editDistance(String s0, String s1) {
        ...
        // >> [AF] How do we compare Strings for equivalence in Java? (see if-statement above)
		else if (s1 == "") {
			return s0.length();
		}
	...
	// >> [AF] Remove test code and main methods before submission (-0.5)
...
// ===================================================
// >>> [AF] Summary
// Functionally excellent submission with a lot to like;
// good delegation into helpers and obviously well-tested,
// though with a good amount of stylistic room for
// improvement as noted above. Fix those for next time and
// you'll be solid!
// ---------------------------------------------------
// >>> [AF] Style Checklist
...
// ---------------------------------------------------
// EditDistUtils:       20 / 20
//   (-3 / missed unit test, max -40)
// DistlePlayer:        268 / 265
//   (-0.5 / missed game under threshold, max -30)
// Correctness Total:   100 / 100
// Style Penalty:       -1.5
// Total:               98.5 / 100
// ===================================================
